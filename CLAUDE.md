...
- **`apply_palette()`** — per-variant color tokens (`CTX_BG`, `A_BG`, `B_BG`, `C_BG`, `GRAD_MIN/MAX`, `Y_BG`, `Z_BG`, `X_BG`, `U_BG`). Variants: `wave`, `dragon`, `lotus`, `lean`, `off`. The `lean` variant is a muted dark monochromatic palette — bgs sit on the sumiInk ramp (#16161D..#363646) so adjacent powerline transitions still read; fgs use Kanagawa accents at low contrast. Powerline structure is identical to wave/dragon/lotus.
