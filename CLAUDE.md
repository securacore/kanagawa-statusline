...
- **`apply_palette()`** — per-variant color tokens (`CTX_BG`, `A_BG`, `B_BG`, `C_BG`, `GRAD_MIN/MAX`, `Y_BG`, `Z_BG`, `X_BG`, `U_BG`). Variants: `wave`, `dragon`, `lotus`, plus a `-lean` form of each (`wave-lean`, `dragon-lean`, `lotus-lean`), and `off`. The `*-lean` variants are muted monochromatic palettes — bgs use a single-family ramp (sumiInk for wave/dragon, lotusWhite for lotus); fgs use that base's accent colors at low contrast. Powerline structure (bg fills + arrows) is identical across all variants.
